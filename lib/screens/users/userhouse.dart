...
class _UserHouseState extends State<UserHouse> {
  ...
  // Fetch user data from Firestore and filter for 'house' category
  Future<void> _fetchUsers() async {
    try {
      ...
      List<Map<String, dynamic>> userList = snapshot.docs
          ...
          .where((user) => user['category'] == 'household')
          .toList(); // Filter for category 'house'

      setState(() {
        _usersList = userList;
        _filteredUsers =
            userList; // Initialize filtered list with 'house' users
      });
    } catch (e) {
